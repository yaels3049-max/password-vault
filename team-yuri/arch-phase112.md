...
## Amendment
AMENDED: 2026-09-14 — **Phase 108 MVP.** `loginUrl` is explicitly configured and authoritative. Phase 112 must not require Phase 108 discovery deferral signals (`phase112Deferred`, `loginIntelligenceHint`, discovery `loginEntryType` / `usesModal`). Missing hints are not a defect. Phase 112 must not invent `loginUrl` and must not reopen Automatic Login Discovery, including withdrawn PayPal M16. See `arch-phase108.md`.
...
| **D-112-5: Discovery deferrals are not required** | arch-phase108 2026-09-14 | Phase 112 must classify without Phase 108 discovery hints. Stored `phase112Deferred` / `loginIntelligenceHint` values are legacy and optional. Phase 112 must not invent `loginUrl` and must not restore Automatic Login Discovery. |
