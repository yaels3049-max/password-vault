...
AMENDED: 2026-07-15 — **Narrow shells ~10% for phone-like proportions.** Reduce shared content-shell **max-width by ~10%** so Digital Home and «הוספת אתרים» / Discover (+ Manage twin) look more phone-shaped and the background reads sharper. Home and Add/Discover **must remain identical width**. Keep max 5 tiles/row. **D-113-27**; AC-113-25 / AC-113-47.

AMENDED: 2026-07-15 — **Credential Details: freeze + header chrome.** Operator UAT: modal «פרטי כניסה» **freezes the app** on use — treat as **blocking defect** (investigate focus trap / render loop / dead ⋮ handlers). **Remove** header ⋮ overflow entirely (non-functional / empty). Place «הגישה פתוחה» + «נעל» **immediately inward of the X** (RTL: X at top-left; lock chip sits to the **right of X**, still on the left side of the modal — not far top-right). Delete moves off the removed kebab to a compact secondary control if still needed. **D-113-28**; AC-113-48…50.

AMENDED: 2026-07-15 — **Remove-site does not survive re-entry (blocking).** Operator: «הסר אתר» in «האתרים שלי» removes from Digital Home for the session, but **after re-login / re-open the sites reappear**. Local UI/IndexedDB remove without durable cloud `user_services` delete (or silent cloud failure + hydrate restoring cloud membership) is a defect vs AC-104-14…16 / AC-109-39. Fix persistence path — do not treat remove as success if cloud membership still exists. **D-113-29**; AC-113-51.
...
| **D-113-17: Digital Home chrome aligned with Discover** | Operator Home screenshot | Digital Home and Discover/Manage **share the same content shell max-width** (single shared CSS token / class). Inside Home, the **icon grid retains phone-like density: max 5 tiles per row**, with intentional side padding. Remove PoC/test fill buttons from Home. Remove marketing subtitle under the title. H1: «הבית הדיגיטלי של {fullName}». Center «ניהול אתרים» CTA (glossary) matching Discover’s «לבית הדיגיטלי». **Amended by D-113-27:** shared max-width is **~10% narrower** than previous shipping width for stronger phone silhouette. |
| **D-113-27: Shell width −10% (phone silhouette)** | Operator 2026-07-15 | Reduce the shared Digital Home–family content shell **max-width by approximately 10%** relative to the current production/desktop shell width, to increase phone-like vertical proportion perception. **Must stay identical** across: Digital Home, «הוספת אתרים» / Discover, and Manage twin screens that already share shell width (D-113-17). Prefer one CSS variable / shared class (do not drift per screen). Keep **max 5 tiles per row**. Expect wave-v2 background to read **sharper / higher quality** at this width (`cover`/`center`; avoid upscaling soft mush — prefer natural cover crop). Login/Auth that already shares the same shell token should inherit; if Login uses a separate width, leave Login alone unless it already tracks the Home shell. Admin out of scope. |
...
| **D-113-20: Shared soft background & radius (landscape shells)** | Operator background image | Apply soft tech background to **Digital Home** and **Manage/Add Sites** content shells. Shells use CSS **border-radius** (≈16–28px) + `overflow: hidden`. Prefer CSS radius over baked bitmap corners. Keep contrast for dark text. **Amended by D-113-26:** approved asset is **wave-v2**; Login/Auth is **in scope**. Admin remains out of 113 unless Phase 107 chooses parity. |
| **D-113-26: Background asset v2 + Login screen** | Operator 2026-07-15 image | Ship operator asset from `team-Yuri/assets/digital-home-shell-wave-v2.png` into app static (`src/assets/backgrounds/` and/or `public/backgrounds/`) as the **sole** Digital Home–family shell background. Replace previous JPG so UAT shows the new soft blue waves / fine lines / dot clusters. Apply on: (1) Digital Home, (2) Manage Sites / Add Sites (wide landscape shells), (3) **Login / Auth entry** (`AuthEntryScreen` / vault entry). Same `background-size: cover` / centered treatment; keep D-113-22 visibility (no heavy white wash). Do **not** require Admin console in this amendment. |
...
2. Order: Header → compact service identity → profile selector → credential fields → compact `פתח כניסה` / `נסה מילוי` (if already present) → Save → secondary Add Profile → delete via compact secondary only (not equal to Save; **no** header kebab).
3. No large full-width Close at bottom; no second Close.

### Header
- Title: `פרטי כניסה`; service name under title when useful.
- **Left cluster (RTL, top):** `X` close (min 40×40, `aria-label="סגירה"`) then **immediately** the vault lock chip «הגישה פתוחה» + «נעל» (D-113-28) — adjacent to X, still on the left side of the modal.
- **No** header ⋮ / overflow / kebab — remove entirely.
- Unsaved close: dialog `השינויים עדיין לא נשמרו` → `המשך עריכה` | `יציאה ללא שמירה` (only if dirty).

### Usability (blocking — D-113-28)
- Modal must remain interactive: type fields, copy, save, close, lock/unlock without freezing the page.
- Investigate and eliminate hang causes (render loops, focus-trap deadlock, backdrop blocking, dead menu handlers).
...
- **Not** via header ⋮ (kebab removed). Use compact secondary control in body/footer if delete remains in product scope.
...
6. **M8 — Background asset v2 + Login:** Copy `team-Yuri/assets/digital-home-shell-wave-v2.png` into app backgrounds; apply to Digital Home, Manage/Add Sites, **and Login/Auth** (D-113-26 / AC-113-33 / AC-113-46). Retire old JPG as the product background. Evidence: screenshots of Home + Login with perceptible new wave pattern.
...
### Review Notes
Credential Details redesign incorporated as D-113-25 (UI-only). Background asset v2 + Login (D-113-26) ready for Manager/Developer as **M8**.

### Required Corrections
None yet (await M7/M8 implementation / UAT).
