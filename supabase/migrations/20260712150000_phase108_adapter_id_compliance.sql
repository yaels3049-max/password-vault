-- Phase 108: align adapter_id with Phase 103 architecture (catalog seed is source of truth)

-- Clear retired interim generic adapter everywhere it may remain.
update public.service_registry
set
  adapter_id = null,
  updated_at = now()
where adapter_id = 'generic';

-- Ensure HTZone keeps its approved site-specific adapter.
update public.service_registry
set
  adapter_id = 'htzone',
  updated_at = now()
where id = 'htzone'
  ...
  and (adapter_id is distinct from 'htzone');

-- Replace ensure RPC so adapter_id always follows the Hub seed on restore
-- (excluded.adapter_id wins — null for generic services, 'htzone' for HTZone).
...
begin
  ...
  set
    ...
    -- Catalog seed is authoritative for adapter routing (Phase 103 / 108).
